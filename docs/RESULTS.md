...
- 源码已改为：模型只定粗细等级，不定连续尺寸。PSO 只做一次闭式微调（评价 ≤1，不搜索）；不准的地方下一轮视觉改。S4 Scripted/LLM 没有 `revise`，第二次求解是同一套等级上的一次微调。改等级要 CachedDrawing/代理视觉。AB4/AB5 极性已翻：消融打开分裂/通信（`vla_ab4_split` / `vla_ab5_comm`）；旧 dump `records_vla_ab4_nosplit.json` / `records_vla_ab5_nocomm.json` 是历史极性，不当作新消融。本页 A2′/主图的 VLA 数字仍来自改前战役（幂律适应度 + 均匀探针 + 多轮通信/分裂），尚未用新初始化重跑；在重跑落地前不要把两轮定稿或「k=2 与 LP 相近不再是结构性的」套到旧表上。
