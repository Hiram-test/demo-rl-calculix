...
这是覆盖度测量，不是科学矛盾：把采样器撑大、专家库跟着长大，这类参数偏移可以被吞掉。
数字照实报；deck 几乎持平，禁止把 E3 写成「监督科学上不行」。
...
对象不是「没见过的孔所以监督差」。无限覆盖一切拓扑的监督库 = 先对每个拓扑类付清 Dörfler-到帽标签，再做一次探针前馈；标签预言机就是对照方法本身。
E4 只测部署时的信息瓶颈：孔/开口在图纸上，h0 探针上孔缘 η² 份额只有几个百分点，一步回归从这根探针发明不了孔缘分级。
母族模型冻结部署是工程协议（单件图纸不会先重采专家库），用来隔离这条瓶颈，不是宣称神经网络学不会孔。
VLA 多出来的不是更大的库，是图纸种子通道。LP/VLA 免训练同场。
...
|---|---|---:|---:|---:|---:|
| bearing_hole | canonical | 0.050 | 0.2368 (0.50, 33.1) | 0.1884 (0.81, 22.8) | 0.2134 (0.95, 22.8) |
| bearing_hole | fp_9600 | 0.064 | 0.2320 (0.52, 35.8) | 0.1843 (0.80, 24.8) | 0.2224 (0.70, 27.7) |
| bearing_hole | fp_9601 | 0.061 | 0.2357 (0.51, 36.1) | 0.1851 (0.81, 25.2) | 0.2093 (0.84, 26.9) |
| deck_opening | canonical | 0.061 | 0.4033 (0.43, 59.8) | 0.2876 (0.90, 34.5) | 0.3648 (0.83, 44.5) |
| deck_opening | fp_9600 | 0.055 | 0.4113 (0.42, 68.0) | 0.2888 (0.91, 37.3) | 0.3583 (0.89, 40.7) |
| deck_opening | fp_9601 | 0.055 | 0.4075 (0.43, 64.9) | 0.2900 (0.91, 36.2) | 0.3548 (0.90, 42.8) |

- bearing_hole（母族 bearing_block）：监督−VLA 中位差距，母族测试集 0.0058 → 探针族 0.0234；监督孔缘尺寸 / VLA 孔缘尺寸中位比 1.34。

- deck_opening（母族 deck_panel）：监督−VLA 中位差距，母族测试集 0.0245 → 探针族 0.0527；监督孔缘尺寸 / VLA 孔缘尺寸中位比 1.52。
...
- 源码已改为：模型只定粗细等级，不定连续尺寸。第一次出网只映射 `GRADE_PRIOR`（`feats=None`，评价 0），可以离谱超/欠预算，这不是要搜索掉的 bug。有上次网格时工具做一次闭式微调（评价 ≤1，不搜索）。S4 Scripted/LLM 没有 `revise`，第二次求解是同一套等级上的一次微调。改等级要 CachedDrawing/代理视觉。AB4/AB5 极性已翻：消融打开分裂/通信（`vla_ab4_split` / `vla_ab5_comm`）；旧 dump `records_vla_ab4_nosplit.json` / `records_vla_ab5_nocomm.json` 是历史极性，不当作新消融。本页 A2′/主图的 VLA 数字仍来自改前战役（幂律适应度 + 均匀探针 + 多轮通信/分裂），尚未用新初始化重跑；在重跑落地前不要把两轮定稿、先验可超预算、或「k=2 与 LP 相近不再是结构性的」套到旧表上。
- 监督专家库由训练实例上的 Dörfler-到帽循环蒸馏（离线求解已计入 A4）；其部署无预算帽语义，canonical 交付停在 44–52% 档位。监督不是 AMR 的替代物，是对已付清的经典循环做压缩。一族 24 专家已经要 149/167 次离线求解；覆盖 M 个拓扑类先付 ≈M×150 次 Dörfler。单件桥梁图纸摊销不起，这才是「为什么不能把全部精力投进监督」的矛盾。无限大、覆盖一切的监督库在定义上已经先跑完了对照方法。
- E3 是采样器支撑外的参数偏移（覆盖度）。E4 允许说的只有：当前探针信息不足，一步回归发明不了孔缘分级；图纸种子是另一条信息通道。禁止把「未见拓扑 / 冻结母族」写成科学结论。失效探针族不进主表、不参与 H1–H4；参考解含孔缘分级线，G3 扫描覆盖其全部记录。
