...
- 源码已改为：主方法是图纸先验 + 一次 PSO（默认 max_solves=2，不许通信/分裂重画），多边形不重画；不再用逐单元等分布按区几何平均涂尺寸。本页 A2′/主图的 VLA 数字仍来自改前战役（幂律适应度 + 均匀探针 + 多轮通信/分裂），尚未用新初始化重跑；在重跑落地前不要把两轮定稿或「k=2 与 LP 相近不再是结构性的」套到旧表上。
