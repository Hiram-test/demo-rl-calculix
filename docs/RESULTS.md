...
- bearing_block：最优 e=0.1673（第 3 次），末轮 e=0.1882，最优后恶化 12.5%；回弹：是。对照：v2 修正版（上节）单调进平台。

- deck_panel：最优 e=0.2758（第 4 次），末轮 e=0.3358，最优后恶化 21.8%；回弹：是。对照：v2 修正版（上节）单调进平台。
