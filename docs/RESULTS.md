...
- 源码已改为：VLA 先画不规则区再由眼睛赋尺寸（没画到的体积也要委派 remainder 尺寸，允许拆剖面），不再用逐单元等分布按区几何平均涂尺寸。本页 A2′/主图的 VLA 数字仍来自改前战役，尚未用新初始化重跑；在重跑落地前不要把「k=2 与 LP 相近不再是结构性的」套到旧表上。其后通信/PSO/分裂/硬帽/就地认证仍是实例循环。
