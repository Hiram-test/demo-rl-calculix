...
- 源码已改为：每次求解后用计算结果和资源存量做下一次分配；PSO 只防止分配超预算。S4 的 `max_solves` 默认是 2，但 Scripted/LLM 头没有 `revise`，战役主路径第一次求解后会停；两次求解只在 CachedDrawing 回放决策 JSON 时发生。AB4/AB5 极性已翻：消融打开分裂/通信（`vla_ab4_split` / `vla_ab5_comm`）；旧 dump `records_vla_ab4_nosplit.json` / `records_vla_ab5_nocomm.json` 是历史极性，不当作新消融。本页 A2′/主图的 VLA 数字仍来自改前战役（幂律适应度 + 均匀探针 + 多轮通信/分裂），尚未用新初始化重跑；在重跑落地前不要把两轮定稿或「k=2 与 LP 相近不再是结构性的」套到旧表上。
