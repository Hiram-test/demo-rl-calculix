...
@dataclass(frozen=True)  # Make planner settings immutable.
...
@dataclass(frozen=True)  # Make planner decisions immutable and auditable.
...
@dataclass(frozen=True)  # Make internal beam nodes immutable.
...
class MultiStepPlanner:  # Search discrete future-hit actions while retaining Dörfler as a fixed candidate.
    ...
    def _stage_cost(self, current: WorldState, prediction: WorldPrediction) -> float:  # Score one predicted transition with risk and resource penalties.
        ...
        uncertainty_term = self.config.uncertainty_weight * prediction.uncertainty  # Penalize epistemic uncertainty.
