...
from .model import RegionAction, ResidualWorldModel  # Import exact-Dörfler and learned world-model actions.
...
def _make_runner(problem: Any, workdir: Path, n_equation_cap: int) -> Any:  # Construct FemRunner across minor repository constructor revisions.
    ...
    for name, parameter in signature.parameters.items():  # Map stable experiment concepts to current argument names.
        ...
        if lower in ("problem", "case"):  # Match the finite-element problem.
            ...
            break  # Fall back to the common positional constructor below.
    else:  # Execute only when all required constructor fields were mapped.
        try:  # Prefer auditable keyword construction.
            return FemRunner(**kwargs)  # Construct the repository runner.
        except TypeError:  # Support older positional-only implementations.
...
def _record_equations(record: Any) -> int:  # Recover measured active equations from a solve record.
    for name in ("n_equations", "equations", "neq"):  # Inspect supported resource fields.
...
def run_exact_dorfler(runner: Any, partition: CachedVisionPartition, *, max_solves: int, n_equation_cap: int, theta: float, refine_factor: float) -> DorflerResult:  # Execute the paired exact-Dörfler trajectory with the same real-solve protocol.
    ...
    hit_count: np.ndarray | None = None  # Initialize unused but consistently measured semantic recurrence.
    ...
    for step in range(max_solves):  # Execute the same maximum number of real solves as world VLA.
        ...
        observation = gateway.observe_solve(adapter.problem, partition, post, record, eta2, hit_count, step)  # Build the same measured state without granting semantic control.
...
def run_bridge_benchmark(output_dir: str | Path, *, smoke: bool = False, max_solves: int = 7, n_equation_cap: int = 120000, theta: float = 0.5, refine_factor: float = 0.5, noninferiority_tolerance: float = 0.03) -> BenchmarkResult:  # Run the clean paired bridge-component benchmark.
    ...
    for name in ("reference", "_reference"):  # Share the trusted reference object when the runner exposes one.
    ...
    world = run_world_model_vla(world_runner, partition=partition, config=WorldVLAConfig(max_solves=max_solves, n_equation_cap=n_equation_cap, theta=theta, refine_factor=refine_factor, artifact_dir=str(root / "world_vla")), model=ResidualWorldModel(), planner=planner)  # Execute the clean world-model VLA route.
